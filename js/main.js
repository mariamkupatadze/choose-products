...
`
let products = JSON.parse(productsJSON).products
...
function createProductCardHtml(product, index) {
    ...
    <div class="card">
      ...
      <div class="card__body">
        ...
        <button productIndex=${index} class="buy--btn">ADD TO CART</button>
      </div>
</main>
    `
    return card;
...
function displayProductsInHtml(productsArray) {
    for( i = 0; i < productsArray.length; i++) {
        let cardHtml = createProductCardHtml(productsArray[i], i)
        productsDiv.innerHTML += cardHtml
    }
}



displayProductsInHtml(products)

let addToCardBtn = document.querySelectorAll(".buy--btn")
let productCart = document.querySelector(".productCart")
let chosenProductsbox = document.querySelector(".chosenProductsbox")

let chosenProducts = localStorage.getItem("chosenProducts")


if(chosenProducts !== null) {
  chosenProducts = JSON.parse(chosenProducts);
  ...
  for (const product of chosenProducts) {
    ...
    <tr>
        ...
        <th scope="row">
        <img width= "80px" src="${product.images[0]}">
        </th>
        <td>"${product.brand}"</td>
        <td>"${product.title}"</td>
        <td>"${product.price}"</td>
        <td> <button class="removeBtn">წაშლა</button> </td>
     
    </tr> 
  
    `
    chosenProductsbox.innerHTML += tr;
...
// add to card button
for(const button of addToCardBtn) {
    button.addEventListener("click", function(){
        let productindex = Number(this.getAttribute("productIndex"));
        let chosenProduct = products[productindex];

        addchosenProductCard(chosenProduct)
    })
}

function addchosenProductCard(product) {
